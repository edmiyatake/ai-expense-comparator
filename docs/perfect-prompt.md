...
Build an Expense Comparator application that lets users manually enter expenses and optionally upload CSVs. Each expense has a date, description, amount, category, and account. Users can create and edit categories, map raw categories, and classify expenses. The system must compare spending across different time periods, including custom date ranges, and detect recurring expenses such as subscriptions by identifying consistent patterns. It should provide visual spending summaries using charts or graphs and generate insights that highlight trends, anomalies, and areas where users can improve financial well-being. The system must store expenses, categories, recurring patterns, and import logs. The UI may be CLI or API based but must allow entering expenses, selecting date ranges, viewing comparisons, and displaying visual reports. Please generate requirements, a domain model, architecture, a full code skeleton, a test skeleton, and a design review covering all these capabilities.