...
class CodeGeneratorAgent(Agent):
    ...
    def run(
        ...
        # --- New: persist skeleton to disk via File tool, if available ---
        if "file" in tools_dict:
            ...
            file_result = tools.invoke(
                ...
                {
                    "operation": "write",
                    # This path is relative to the FileTool's sandbox root (generated/)
                    "path": "artifacts/code_skeleton.md",
            ...
            if io:
                io.log(f"[{self.name}] File tool not registered; skipping file write.")
